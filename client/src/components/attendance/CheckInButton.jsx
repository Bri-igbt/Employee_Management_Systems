    ...
    return (
        <div className='absolute bottom-4 right-4 flex flex-col z-1'>
            <button 
                ...
                disabled={loading} 
                className={`w-full max-w-xs flex justify-between items-center gap-8 p-4 rounded-xl bg-gradient-to-br text-white ${isCheckedIn ? "from-slate-700 to-slate-900" : "from-orange-600 to-orange-700"}`}>
                {loading ? 
                    <Loader2Icon className='size-7 animate-spin'/> 
                    : isCheckedIn ? <LogOutIcon className='size-7' /> 
                    : <LogInIcon className='size7'/> 
                }
