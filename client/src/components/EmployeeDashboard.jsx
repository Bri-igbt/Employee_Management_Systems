            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8'>
                {cards.map((item, index) => (
                    <div key={index} className='card card-hover p-5 sm:p-6 relative overflow-hidden group flex items-center justify-between'>
                        ...
                        <item.icon className='size-10 p-2.5 rounded-lg bg-slate-100 text-slate-600 group-hover:bg-orange-50 group-hover:text-orange-600 transition-colors duration-200' />
                    </div>
                ))}
            </div>

            <div className='flex flex-col sm:flex-row gap-3'>
                <Link to="/attendance" className='btn-primary text-center inline-flex gap-2 items-center justify-center'>
